...
export class AllExceptionsFilter implements ExceptionFilter {
		...
		if (exception instanceof HttpException) {
			...
			responseMessage =
				foundErrorPrompt === null ? "Internal Server Error" : JSON.stringify(foundErrorPrompt.value);
		} else {
			status = HttpStatus.INTERNAL_SERVER_ERROR;
			errorType = "Server Error";
			errorMessage = String(exception);
		...
		const errorResponse = this.getErrorResponse(status, errorType, message, req);
	...
	customiseErrorMsgByLang(ErrorPrompt: ErrorPrompt, errorMsg: string, lang: string = "en") {
		let message = "";
